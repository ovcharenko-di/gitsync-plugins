...
Процедура ПередНачаломВыполнения(ПутьКХранилищу, КаталогРабочейКопии) Экспорт // BSLLS:MissingParameterDescription-off
...
Функция КомандаRing(КаталогВыгрузки, РабочееПространство, КаталогПроекта)
	...
	Команда.УстановитьКоманду(ИсполняемыйФайл);

	Команда.ДобавитьПараметр(ИмяМодуля);
...
Функция КомандаEdtcli(КаталогВыгрузки, РабочееПространство, КаталогПроекта)
	...
	Команда.ДобавитьПараметр(РабочееПространство);
